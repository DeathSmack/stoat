...
class MonochromeSource implements SourceInstance {
  ...
  public async search(
    ...
    if (searchType === 'track' && (response.data?.tracks?.items ?? response.data?.items)) {
      for (const t of (response.data?.tracks?.items ?? response.data?.items)) {
    ...
    } else if (searchType === 'video' && (response.data?.videos?.items ?? response.data?.items)) {
      for (const v of (response.data?.videos?.items ?? response.data?.items)) {
  ...
  public async getTrackUrl(track: TrackInfo): Promise<TrackUrlResult> {
    ...
    // Videos still use TIDAL manifest flow
    if (isVideo) {
      ...
      if (!uri)
        return {
          exception: {
            message: 'Failed to extract playable URI from video manifest',
            severity: 'fault'
          }
        }
      ...
      return {
        url: uri,
        protocol: uri.includes('.m3u8') ? 'hls' : 'dash',
        format: 'audio/aac',
      }
    }

    // Tracks: use Amazon Music via stream-resolver sidecar
    const sidecarUrl = process.env.STREAM_RESOLVER_URL || 'http://127.0.0.1:3100'

    // Fetch track metadata from instance for Amazon lookup
    const metaRes = await this.fetchWithRetry<any>(
      `/info/?id=${track.identifier}`
    )

    const meta = metaRes?.data || metaRes
    const trackTitle = meta?.title || track.title
    const artist = meta?.artist?.name || meta?.artists?.[0]?.name || track.author
    const album = meta?.album?.title || meta?.albumTitle || ''
    const duration = (meta?.duration || track.length / 1000) as number
    const replayGain = meta?.replayGain
    const peakAmplitude = meta?.peak

    logger(
      'debug',
      'Monochrome',
      `Resolving Amazon stream for "${trackTitle}" by ${artist} (${Math.round(duration)}s)`
    )

    // Map quality config to Amazon quality
    const qualityMap: Record<string, string> = {
      'LOW': 'SD_LOW',
      'HIGH': 'SD_HIGH',
      'LOSSLESS': 'HD_44',
      'HI_RES': 'HD_44',
      'HI_RES_LOSSLESS': 'HD_44',
    }
    const amazonQuality = qualityMap[this.config.quality || 'LOW'] || 'SD_LOW'
    ...
    try {
      const { body, statusCode } = await http1makeRequest(
        `${sidecarUrl}/stream?${streamParams.toString()}`,
        { method: 'GET', timeout: 15000 }
      )

      if (statusCode !== 200 || !body) {
        logger('warn', 'Monochrome', `Sidecar returned ${statusCode}`)
        return {
          exception: {
            message: `Stream resolver returned status ${statusCode}`,
            severity: 'fault'
          }
        }
      }
      ...
      if (!result.streamUrl) {
        return {
          exception: {
            message: 'Stream resolver returned no URL',
            severity: 'fault'
          }
        }
      }
      ...
      logger(
        'info',
        'Monochrome',
        `Got Amazon stream: ${result.quality} (${result.streamUrl.substring(0, 80)}...)`
      )

      // Determine protocol from URL
      let protocol: string = 'http'
      if (result.streamUrl.includes('.mpd')) protocol = 'dash'
      else if (result.streamUrl.includes('.m3u8')) protocol = 'hls'
      ...
      return {
        exception: {
          message: `Stream resolver error: ${e instanceof Error ? e.message : String(e)}`,
          severity: 'fault'
        }
      }
    }
